...
public enum APIError: ApplicationError {
    case FailedResponse(_ message: String, _ validations: [String: String])
    case UnauthorizedRequest(_ message: String)
    case PasswordExpired(_ message: String)
    ...
    public var details: ErrorDetails {
        switch self {
        case .FailedResponse(let message, let validations):
            return .init(
        ...
        case .InvalidUrl(let url):
            let message = String.error.InvalidUrl
        ...
        case .FailedRequest(let debugMessage):
            let message = String.error.FailedRequest
